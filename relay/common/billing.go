...
type BillingSettler interface {
	...
	// GetChargedQuotas returns the durable amounts charged to the funding source
	// and token after the latest settlement attempt. The values may differ when
	// one side of settlement succeeds and the other fails.
	GetChargedQuotas() (fundingQuota int, tokenQuota int)
