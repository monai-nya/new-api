...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if topUp.Status == common.TopUpStatusPending {
			// RechargeEpay completes the order in a single DB transaction with a row
			// lock, so duplicate webhook deliveries (especially across instances)
			// cannot both observe StatusPending and double-credit the user.
			quotaToAdd, rechargeErr := model.RechargeEpay(verifyInfo.ServiceTradeNo, verifyInfo.Type, c.ClientIP())
			if rechargeErr != nil {
				logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 充值失败 trade_no=%s user_id=%d client_ip=%s error=%q topup=%q", topUp.TradeNo, topUp.UserId, c.ClientIP(), rechargeErr.Error(), common.GetJsonString(topUp)))
				return
			}
